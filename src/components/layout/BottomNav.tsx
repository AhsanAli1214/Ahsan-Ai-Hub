    ...
    <nav className="btm-nav md:hidden border-t bg-background/80 backdrop-blur-lg safe-area-bottom h-20">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          side="top"
          className="w-56 mb-4 p-2 rounded-[2rem] border-primary/20 bg-background/95 backdrop-blur-xl shadow-2xl animate-in slide-in-from-bottom-2 duration-300"
        >
          {moreItems.map((item) => (
            <DropdownMenuItem key={item.href} asChild>
              <Link
                href={item.href}
                className="flex items-center gap-3 p-3 rounded-2xl cursor-pointer hover:bg-primary/10 transition-colors group"
              >
